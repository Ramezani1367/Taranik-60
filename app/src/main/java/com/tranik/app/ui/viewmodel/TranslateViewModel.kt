...
class TranslateViewModel @Inject constructor() : BaseViewModel() {
    ...
    fun swapLanguages() {
        _state.update {
            it.copy(
                ...
                result = null,
                inputText = it.result ?: it.inputText
            )
        }
    }
        ...
        viewModelScope.launch {
            try {
                val result = withContext(Dispatchers.IO) {
                    // استفاده از MyMemory API رایگان (بدون API key)
                    val url = "https://api.mymemory.translated.net/get?q=" +
                            java.net.URLEncoder.encode(text, "UTF-8") +
                            "&langpair=${_state.value.sourceLang}|${_state.value.targetLang}"
                ...
                _state.update {
                    it.copy(
                        ...
                        ) + it.history.take(19) // حداکثر ۲۰ مورد
                    )
                }
            } catch (e: Exception) {
                _state.update {
                    it.copy(
                        isLoading = false,
                        error = e.message ?: "خطا در ترجمه. اتصال اینترنت را بررسی کنید."
                    )
                }
            }
        }
    ...
    private fun parseTranslationResponse(json: String): String {
        // پارس ساده JSON بدون کتابخانه اضافی
